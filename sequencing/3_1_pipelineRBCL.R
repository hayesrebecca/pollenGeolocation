...
ncbi <- read.csv("pipeline/R2023/RBCL/rbcl_classified_NCBI.csv")
...
rdp <- read.table("pipeline/R2023/RBCL/rbcl_classified_rdp.txt",
                  sep="\t", header=FALSE)

## loading old rdp run to pull column names from
rdp_old <- read.table("../skyIslands_saved/SI_pipeline/merged/RBCL/rbcl_classified_rdp_old.txt",
                  sep="\t", header=TRUE)
...
plants <- read.csv("data/relational/relational/tables/plant.csv")
...
not_in_veg <- unique(ids$FinalGenus[ids$FinalGenusInVeg == 0])
...
# based on USDA plant database - these genera are represented in Oregon, but keep in mind that many of the spp hits we are getting are not found there
usda_plants <- read.csv("pipeline/USDA_plants_lane_county_OR.csv", header=TRUE)

# Extract first word and get unique values
acceptable <- unique(sub(" .*", "", usda_plants$Scientific.Name))

ids$GenusInUSDAPlants <- ifelse(ids$FinalGenus %in% acceptable, 1, 0)
ids$rdpGenusInUSDAPlants <- ifelse(ids$GenusRDP %in% acceptable, 1, 0)
...
write.csv(ids, "pipeline/R2023/RBCL/ID-Manual-RH2.csv",row.names=FALSE) # worked on this file doing manual calls
