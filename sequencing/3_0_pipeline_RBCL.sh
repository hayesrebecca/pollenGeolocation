...
cd ../mnt/pipeline/R2023

qiime tools export --input-path dada2-RBCL/rep-seqs-dada2-RBCL.qza --output-path RBCL
...
cd ~/../../Volumes/bombus/rhayes/Dropbox\ \(University\ of\ Oregon\)/pnw_survey_saved/pipeline

#this one gives taxonomy to species (we ended up only using this one)
java -Xmx2g -jar classifiers/RBCLclassifierRDP/qiime-master-rdp_classifier_2.2/rdp_classifier_2.2/rdp_classifier-2.2.jar classify -t classifiers/RBCLclassifierRDP/rbcL_rdp_trained/rbcL.properties -o R2023/RBCL/rbcl_classified_rdp.txt -q R2023/RBCL/rep-seqs-RBCL.fasta
...
docker run -itv /Volumes/bombus/rhayes/Dropbox\ \(University\ of\ Oregon\)/pnw_survey_saved/pipeline:/mnt/pipeline ncbi/blast
cpan App::cpanminus
...
cd ../mnt/pipeline/R2023 #[or whichever run you're working on]
...
## that done, navigate to your pipeline folder and run the script
...
perl ../classifiers/RBCLclassifierNCBI/BlastToTableexistingdb.pl RBCL/rep-seqs-RBCL.fasta ../classifiers/RBCLclassifierNCBI/2023_rbcL_only_DB > RBCL/rbcl_classified_NCBI.txt

## TODO pickup here

## top 10 matches
perl ../classifiers/RBCLclassifierNCBI/BlastToTableexistingdb_top_10.pl RBCL/rep-seqs-RBCL.fasta ../classifiers/RBCLclassifierNCBI/2023_rbcL_only_DB > RBCL/rbcl_classified_NCBI_top_10.txt
...
## CHECK IF CONTROLS ARE STILL IN tableRBCL
qiime feature-table summarize \
  --i-table ../dada2-RBCL/tableRBCL.qza \
  --o-visualization table0summary.qzv \
  --m-sample-metadata-file ../maps/pnw2023mapRBCL.txt
  
#cd merged/RBCL

qiime feature-table filter-features --i-table ../dada2-RBCL/tableRBCL.qza --m-metadata-file featurestokeep2023.txt --o-filtered-table 2023tablefilt1.qza
## CHECK CONTROLS ARE STILL IN TABLE1FILT
qiime feature-table summarize \
  --i-table 2023tablefilt1.qza \
  --o-visualization table1summary.qzv \
  --m-sample-metadata-file ../maps/pnw2023mapRBCL.txt
 
## FILTER 2: filter out all sequences that came out in less than 2 samples


qiime feature-table filter-features --i-table 2023tablefilt1.qza --p-min-samples 2 --o-filtered-table 2023tablefilt2.qza


## CHECK CONTROLS ARE STILL IN TABLE2FILT
qiime feature-table summarize \
  --i-table 2023tablefilt2.qza \
  --o-visualization table2summary.qzv \
  --m-sample-metadata-file ../maps/pnw2023mapRBCL.txt

# REFILTER REPSEQS

qiime feature-table filter-seqs --i-data ../dada2-RBCL/rep-seqs-dada2-RBCL.qza --i-table 2023tablefilt2.qza --o-filtered-data rep-seqs-RBCL-filtered2023.qza

qiime feature-table tabulate-seqs --i-data rep-seqs-RBCL-filtered2023.qza --o-visualization rep-seqs-RBCL-filtered2023.qzv
qiime tools view rep-seqs-RBCL-filtered2023.qzv
...
qiime alignment mafft --i-sequences rep-seqs-RBCL-filtered2023.qza --o-alignment aligned_repseqsRBCL2023.qza

#Then filter out the unconserved, highly gapped columns from alignment
qiime alignment mask --i-alignment aligned_repseqsRBCL2023.qza --o-masked-alignment masked_aligned_repseqsRBCL2023.qza
...
qiime phylogeny fasttree --i-alignment masked_aligned_repseqsRBCL2023.qza --o-tree unrooted_treeRBCL2023.qza

#now root it
qiime phylogeny midpoint-root --i-tree unrooted_treeRBCL2023.qza --o-rooted-tree rooted-treeRBCL2023.qza
...
## NCASI2023: only one pollen round so can filter directly on table2filt
...
## qiime taxa barplot --i-table tablef3.qza  --i-taxonomy taxonomyRBCL.qza --m-metadata-file ../maps/2023pollensamples.txt --o-visualization taxa-bar-plots2023.qzv

## 2023 all controls were already filtered out because the features therein were filtered out at previous steps, removing contaminants automatically from all samps due to poor quality of those sequences
...
## 2023 check if controls were filtered out

qiime taxa barplot --i-table 2023tablefilt2.qza --i-taxonomy taxonomyRBCL.qza --m-metadata-file ../maps/pnw2023mapRBCL.txt --o-visualization taxa-bar-plots_f2.qzv
...
## want to maximize number of samples and features tradeoff. Minimum 80% of samples kept

# R1
qiime feature-table summarize --i-table 2023tablefilt2.qza --o-visualization tablef2.qzv

qiime diversity alpha-rarefaction --i-table 2023tablefilt2.qza --i-phylogeny rooted-treeRBCL2023.qza --m-metadata-file ../maps/pnw2023mapRBCL.txt --o-visualization alphararefactRBCL2023.qzv --p-max-depth 8039
# 8039, Retained 1,543,488 (33.85%) features in 192 (80.00%) samples at the specifed sampling depth out of 240 samples
...
qiime diversity core-metrics-phylogenetic --i-phylogeny rooted-treeRBCL2023.qza --i-table 2023tablefilt2.qza --p-sampling-depth 8039 --m-metadata-file ../maps/pnw2023mapRBCL.txt --output-dir final/core_metricsRBCL --verbose
